...
public class TopRuler extends HorizontalRuler {
  ...
  private void drawScale(Canvas canvas) {
    for (float i = outerRuler.getMinScale(); i <= outerRuler.getMaxScale(); i++) {
      ...
      if (locationX > getScrollX() - drawOffset && locationX < (getScrollX() + canvas.getWidth()
          + drawOffset)) {
        if (i % outerRuler.getCount() == 0) {
          canvas.drawLine(locationX, 0, locationX, outerRuler.getBigScaleHeight(), bigSmallPaint);
          canvas.drawText(String.valueOf(i / 10), locationX,
              outerRuler.getBigScaleHeight() + outerRuler.getTextMargin(), textPaint);
        } else {
          canvas.drawLine(locationX, 0, locationX, outerRuler.getSmallScaleHeight(),
              smallScalePaint);
        }
        //画轮廓线
